...
const FEEDS = [
  {
    ...
    description:
      'Cannabis news, culture, and community from 100+ curated accounts. Powered by Cannect.',
  },
  {
    shortname: 'cannect',
...
async function main() {
  ...
  for (const feed of FEEDS) {
    ...
    try {
      ...
      console.log(
        `✅ Registered: at://${agent.session.did}/app.bsky.feed.generator/${feed.shortname}`
      );
    } catch (error) {
      if (error.message?.includes('already exists')) {
        ...
        await agent.api.com.atproto.repo.putRecord(record);
        console.log(
          `✅ Updated: at://${agent.session.did}/app.bsky.feed.generator/${feed.shortname}`
        );
      } else {
